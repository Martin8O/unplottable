...
4. **Review** — human reads; iterate until happy.
5. **Wrap-up** (on "done") — re-run gate; ADR row if a decision changed; changelog entry; bootstrap head update; model-fit retro line; scoped commit + push.
...
Model/effort per step follows the tier tags in the plan (`Local/all prompts.md`, private) via the shared model ladder.
